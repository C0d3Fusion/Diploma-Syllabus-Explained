...
<style>
  ...
  .equation {
    ...
    padding: 10px;
    margin: 15px 0;
    overflow-x: auto; /* For scroll if equation is too long */
  }
  .equation code {
...
<ul>
    <li><b>Infinite Input Impedance ($Z_{in} = \infty$):</b> No current flows into either input terminal (non-inverting or inverting). This means the op-amp doesn't load the source connected to its inputs.</li>
    <li><b>Zero Output Impedance ($Z_{out} = 0$):</b> The op-amp can supply any amount of current to the load without any voltage drop across its internal resistance. The output voltage remains independent of the load.</li>
    <li><b>Infinite Open-Loop Voltage Gain ($A_{OL} = \infty$):</b> Even a tiny difference in voltage between the input terminals results in a huge, theoretically infinite, output voltage. This high gain is crucial for negative feedback applications.</li>
    <li><b>Infinite Bandwidth ($BW = \infty$):</b> The op-amp can amplify signals of any frequency without attenuation.</li>
    <li><b>Zero Offset Voltage ($V_{offset} = 0$):</b> When both input terminals are at the same voltage (e.g., grounded), the output voltage is exactly zero.</li>
    <li><b>Zero Common-Mode Rejection Ratio ($CMRR = \infty$):</b> The op-amp amplifies only the difference between the input signals and perfectly rejects any common-mode signal (a signal present on both inputs simultaneously).</li>
</ul>
...
<ul>
    ...
    <li><b>High (but finite) Open-Loop Voltage Gain:</b> Very high, often in the range of $10^5$ to $10^6$.</li>
    ...
    <li><b>Non-zero Input Offset Voltage ($V_{os}$):</b> A small voltage difference between the inputs is required to make the output zero. This can be compensated for in many op-amps.</li>
    <li><b>Non-zero Input Bias Current ($I_B$):</b> Small currents flow into the input terminals to bias the internal transistors.</li>
...
<ol>
    <li><b>Open-Loop Configuration:</b>
        <ul>
            ...
            <li>Due to the extremely high open-loop gain, even a tiny difference at the input will drive the output to one of the supply rails (either positive saturation $V_{sat+}$ or negative saturation $V_{sat-}$).</li>
    ...
    <li><b>Closed-Loop Configuration:</b>
        <ul>
            ...
            <li><b>Key Assumption for Closed-Loop Analysis (Ideal Op-Amp):</b>
                <ul>
                    <li><b>Virtual Short/Virtual Ground:</b> Due to the infinite open-loop gain and negative feedback, the voltage difference between the inverting and non-inverting input terminals is virtually zero ($V_{in-} \approx V_{in+}$). If the non-inverting input is grounded, the inverting input is considered a "virtual ground."</li>
                    <li><b>No current into input terminals:</b> As mentioned before, $I_{in-} = I_{in+} = 0$.</li>
                </ul>
            </li>
        </ul>
    </li>
...
<ul>
    <li><b>Inverting Amplifier:</b>
        <ul>
            <li><b>Configuration:</b> The input signal is applied to the <b>inverting input</b> through an input resistor ($R_{in}$), and feedback is provided from the output to the inverting input through a feedback resistor ($R_f$). The non-inverting input is grounded.</li>
            <li><b>Gain Equation:</b>
                <div class="equation"><code>$A_v = -\frac{R_f}{R_{in}}$</code></div>
            </li>
            <li><b>Characteristics:</b> The output signal is $180^\circ$ out of phase with the input signal (hence "inverting"). The gain is controlled by the ratio of the feedback and input resistors.</li>
            <li><b>Input Impedance:</b> Approximately $R_{in}$.</li>
            <li><b>Output Impedance:</b> Very low (ideally zero).</li>
        </ul>
    </li>
    <li><b>Non-Inverting Amplifier:</b>
        <ul>
            <li><b>Configuration:</b> The input signal is applied directly to the <b>non-inverting input</b>. Feedback is provided from the output to the inverting input through a voltage divider formed by $R_f$ and $R_1$.</li>
            <li><b>Gain Equation:</b>
                <div class="equation"><code>$A_v = 1 + \frac{R_f}{R_1}$</code></div>
            </li>
            ...
            <li><b>Output Impedance:</b> Very low (ideally zero).</li>
            <li><b>Voltage Follower (Buffer):</b> A special case of the non-inverting amplifier where $R_f = 0$ (short) and $R_1 = \infty$ (open), resulting in a gain of 1. It provides high input impedance and low output impedance, making it ideal for isolating stages and impedance matching.</li>
        </ul>
    </li>
</ul>
...
<ul>
    <li><b>Configuration:</b> Multiple input signals are applied to the <b>inverting input</b> through separate input resistors ($R_1, R_2, ..., R_n$). A feedback resistor ($R_f$) connects the output to the inverting input. The non-inverting input is grounded.</li>
    ...
    <li><b>Output Voltage:</b>
        <div class="equation"><code>$V_{out} = -R_f \left( \frac{V_1}{R_1} + \frac{V_2}{R_2} + ... + \frac{V_n}{R_n} \right)$</code></div>
    </li>
    <li><b>Special Case (Equal Resistors):</b> If $R_1 = R_2 = ... = R_n = R$ and $R_f = R$, then $V_{out} = -(V_1 + V_2 + ... + V_n)$. This configuration sums the inputs directly and inverts the result.</li>
</ul>

<h3>3. Differentiator</h3>
<ul>
    <li><b>Configuration:</b> An input capacitor ($C_{in}$) is connected in series with the input signal and the inverting terminal. A feedback resistor ($R_f$) connects the output to the inverting terminal. The non-inverting input is grounded.</li>
    ...
    <li><b>Output Voltage:</b>
        <div class="equation"><code>$V_{out} = -R_f C_{in} \frac{dV_{in}}{dt}$</code></div>
    </li>
...
<h3>4. Integrator</h3>
<ul>
    <li><b>Configuration:</b> An input resistor ($R_{in}$) is connected in series with the input signal and the inverting terminal. A feedback capacitor ($C_f$) connects the output to the inverting terminal. The non-inverting input is grounded.</li>
    <li><b>Operation:</b> The output voltage is proportional to the integral of the input voltage with respect to time.</li>
    <li><b>Output Voltage:</b>
        <div class="equation"><code>$V_{out} = -\frac{1}{R_{in}C_f} \int V_{in} dt$</code></div>
    </li>
    <li><b>Characteristics:</b>
        <ul>
